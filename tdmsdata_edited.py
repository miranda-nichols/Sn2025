...
def _get_prop_ci(obj, key, default=None):
        """Case-insensitive property getter for nptdms objects."""
...
class TdmsData(object):
    ...
    def __init__(self,path):
        ...
        for g in self.groups:
            ...
            desc = _get_prop_ci(g, 'Description', default='')
            self.groupdescriptions[g.name] = desc if desc is not None else ''
    ...
    def __str__(self) :
        ...
        for g in self.groupchannels.keys() :
            desc = self.groupdescriptions.get(g, '')
            s += (g + ' - ' + str(desc) + ' - ' + str(self.groupchannels[g]) + '\n')
        return s
